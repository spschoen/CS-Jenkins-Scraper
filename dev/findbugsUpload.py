        ...
        if node.hasAttribute("category"):
            ...
            for classNode in node.childNodes:
                ...
                if cur.rowcount == 0: #The values do not exist in the testMethodUID table
                    #Check if classUID values at least exist, which may not be possible if methodUID doesn't exist, but it's double checking.                cur.execute("SELECT * FROM classUID WHERE Package = %s and Class = %s",(package, className))
